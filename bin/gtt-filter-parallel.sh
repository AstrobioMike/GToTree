...
if [ $nucleotide != 'false' ]; then
    target_gene_suffix=".fa"
else
    target_gene_suffix=".faa"
fi


gtt-count-bases-per-seq -i ${tmp_dir}/${1}_hits${target_gene_suffix} -o ${tmp_dir}/${1}_Num_bps.tmp
...
gtt-filter-seqs-by-length -i ${tmp_dir}/${1}_hits${target_gene_suffix} -m $min_len_rnd -M $max_len_rnd -o ${tmp_dir}/${1}_hits_filtered.tmp > ${tmp_dir}/${1}_filter.out.tmp
