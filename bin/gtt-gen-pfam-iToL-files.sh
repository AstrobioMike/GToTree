...
do
    ...
    awk -F $'\t' -v col="$target_col" ' $col > 0 { print $1 } ' ${output_dir}/Pfam_search_results/Pfam-hit-counts.tsv | tail -n +2 > ${tmp_dir}/Genomes_with_hits_to_${target}.tmp
    ...
    if [ -s ${tmp_dir}/Genomes_with_hits_to_${target}.tmp ]; then
        ...
        awk -F $'\t' ' $8 == "No" { print $1 } ' ${output_dir}/Genomes_summary_info.tsv | sort > ${tmp_dir}/sorted_genomes_to_leave_out_of_Pfam_iToL_files.tmp

        if [ -s ${tmp_dir}/sorted_genomes_to_leave_out_of_Pfam_iToL_files.tmp ]; then
            comm -23 <( sort ${tmp_dir}/Genomes_with_hits_to_${target}.tmp ) ${tmp_dir}/sorted_genomes_to_leave_out_of_Pfam_iToL_files.tmp > ${tmp_dir}/genomes_retained_for_${target}_iToL.tmp
        else
        ...
        cat <(sed 's/$/ branch node #0000ff 3 normal/' ${tmp_dir}/genomes_for_iToL_for_${target}.tmp) >> ${output_dir}/Pfam_search_results/iToL_files/${target}-iToL.txt
