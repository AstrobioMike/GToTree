    ...
    run:
        for acc in accessions:
            ...
            with open(status_path, 'r') as f:
                for line in f:
                    acc, status, downloaded, prodigal_used, final_AA_path, num_genes, final_nt_path = line.strip().split('\t')
...
rule process_ncbi_accessions:
    ...
    run:
        ...
        if done:
            done, final_AA_path, num_genes, final_nt_path = filter_and_rename_fasta(acc_gd.id, run_data, run_data.ncbi_downloads_dir)
        else:
            final_AA_path = None

        with open(output[0], 'w') as f:
            f.write(f'{wildcards.acc}\t{int(done)}\t{int(downloaded)}\t{int(prodigal_used)}\t{final_AA_path}\t{num_genes}\t{final_nt_path}\n')
