...
rule all:
    input:
        expand(f"{run_data.found_SCG_seqs_dir}/{{SCG}}-genomes-with-hits-after-filtering.txt", SCG=SCG_ids)
    run:
        count_dict = {genome_id: 0 for genome_id in genome_ids}
        for SCG in SCG_ids:
            num_genomes_with_hits = 0
            with open(f"{run_data.found_SCG_seqs_dir}/{SCG}-genomes-with-hits-after-filtering.txt", 'r') as f:
                for line in f:
                    genome_id = line.strip()
                    count_dict[genome_id] += 1
                    num_genomes_with_hits += 1

            SCG_target = SCG_dict[SCG]
            SCG_target.gene_length_filtered = True
            SCG_target.num_genomes_with_hits_after_len_filtering = num_genomes_with_hits
...
rule filter_genes:
    output:
        f"{run_data.found_SCG_seqs_dir}/{{SCG}}-genomes-with-hits-after-filtering.txt"
    run:
