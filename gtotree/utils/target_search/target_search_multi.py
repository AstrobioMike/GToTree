"""
The combined driver behind `gtt search-annotations`.

`search-annotations` is the one command for searching input genomes for target
annotations -- Pfam domains (`-p`) and/or KEGG Orthologs (`-K`). It supersedes the
older per-type `search-pfams` / `search-kos` commands: give it one flag and it does
exactly what one of those did, give it both and it does both over a *single* genome
preprocessing pass instead of preprocessing every genome twice.

How this relates to the single-spec code in `target_search_cli.py`:

  * The per-type behavior still lives entirely in `TargetSearchSpec` (one spec per
    target type) and the shared stage/setup/output helpers. This driver drives a
    *list* of specs rather than one, but every spec-specific step -- collecting
    targets, pressing profiles, writing the counts table, combining hit seqs -- is the
    same function the single-spec path calls.
  * The genome phases (resolve inputs, look up NCBI accessions, preprocess) are
    ...
    a genome for both in the worker that produced its proteins -- that fused worker is
    unchanged.

Output layout: each target type gets its own subdirectory under the output directory
(`<out>/pfam/`, `<out>/ko/`), each identical to what a single-type run would have put
at the top level. Run-level files that aren't type-specific (the removed-genomes
report, the genomes summary, phase-stats) live at the top.
"""
...
# the target types this command can search for, in the order they appear in output and
# reporting. Each name is a key into target_search_spec.get_spec().
SPEC_NAMES = ["pfam", "ko"]

# per-type output subdirectory under the output directory
...
RESUME = ResumeProfile(
    ...
    # database versions aren't known until fetched, so a run interrupted before then
    # legitimately has None stored
...
def build_parser(parent_subparsers=None):
    ...
    desc = ("This is a helper program that searches a set of input genomes for target "
            "annotations -- Pfam domains and/or KEGG Orthologs (KOs). It takes the "
            "same genome inputs as the main GToTree program, preprocesses them the "
            "same way, and produces the same Pfam/KO search results a full GToTree run "
            "would have produced if you'd passed `-p` and/or `-K` to it. Give both "
            "flags and every genome is preprocessed once and searched for both.")

    example = ("Ex. usage: `gtt search-annotations -f my-genomes.txt "
               "-p my-pfams.txt -K my-kos.txt`")
...
def run_search(args, specs=None):  # pragma: no cover
    ...
    # one run-level genomes summary at the top, using any active spec for the
    # per-genome search-completion column (they share the genome set)
    summary_path = outputs.write_genomes_summary(out_dir, run_data, active[0][1])
